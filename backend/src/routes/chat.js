...
const { assignUserForQueue, getDefaultQueueForInstance, userCanAccessQueue } = require('../services/queueDistribution');
...
router.get('/conversations', async (req, res) => {
  try {
    ...
    if (!canSeeAll) {
      query += ` AND (c.assigned_user_id = $${p++} OR c.assigned_user_id IS NULL)`;
      params.push(user.id);
    }
...
router.post('/conversations', async (req, res) => {
  try {
    ...
    let { rows } = await pool.query('SELECT * FROM conversations WHERE phone = $1', [normalized]);
    if (!rows.length) {
      const { rows: nr } = await pool.query(
        `INSERT INTO conversations (phone, client_name, status, last_message_at) VALUES ($1,$2,'open',NOW()) RETURNING *`,
        [normalized, clientName || normalized]
      );
      rows = nr;
    }
    res.json(rows[0]);
  ...
  try {
    ...
    const { rows: msgRows } = await pool.query(
      ...
      [conv.id, zapiMessageId, user.id, user.name, type,
       type === 'text' ? text.trim() : null, null, fileName || null, caption || null]
    );
...
router.get('/unread', async (req, res) => {
  try {
    ...
    const params = [];
    if (!canSeeAll) { query += ' AND (assigned_user_id=$1 OR assigned_user_id IS NULL)'; params.push(user.id); }
    const { rows } = await pool.query(query, params);
